...
//      checkAdmin(session);
  ...
  @GetMapping("/{id}/edit")
    public String showEditNotice(@PathVariable(name = "id") Long id, Model model, HttpSession session) {
//      checkAdmin(session);
      Notice noticeById = noticeService.getNoticeById(id);
      model.addAttribute("notice", noticeById);
      return "editNotice";
  ...
  @PostMapping("/{id}/edit")
    public String updateNotice(@PathVariable(name = "id") Long id, @ModelAttribute NoticeRequest noticeRequest) {
